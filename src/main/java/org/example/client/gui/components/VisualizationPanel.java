...
public class VisualizationPanel extends JPanel {
    ...
    private void initContextMenu() {
        ...
        editMenuItem = new JMenuItem(localization.getString("button.update"));
    ...
    private void handleDoubleClick(MouseEvent e) {
        Product clickedProduct = findProductAtPoint(e.getPoint());
        if (clickedProduct != null) {
            selectedProduct = clickedProduct;
            createSelectionEffect(selectedProduct);
            editSelectedProduct(); // Прямое редактирование при двойном клике
        }
    }
    private Product findProductAtPoint(Point p) {
    ...
    private void editSelectedProduct() {
        if (selectedProduct.getCreatorId() == SessionState.getInstance().getCurrentUser().getId()) {
    ...
    private void showProductInfo(Product product) {
        ...
        Object[] options = {
                localization.getString("button.edit"), // Кнопка редактирования
                localization.getString("button.close") // Кнопка закрытия
        };
    ...
    public void highlightProduct(Product product) {
        ...
        // Прокрутить к выбранному продукту
        Point position = convertToScreen(product.getCoordinates());
        Rectangle viewRect = getVisibleRect();
        viewRect.setLocation(position.x - viewRect.width/2, position.y - viewRect.height/2);
    ...
    private void drawAxisLabels(Graphics2D g2d, int centerX, int centerY) {
        ...
        // Фиксированный шаг сетки - 50 единиц
        int gridStep = 50;

        // Вычисляем диапазон для отображения подписей
        int minX = (int) ((-centerX + PADDING) / scale);
        int maxX = (int) ((getWidth() - centerX - PADDING) / scale);
        int minY = (int) ((-getHeight() + centerY + PADDING) / scale);
        int maxY = (int) ((centerY - PADDING) / scale);

        // Рисуем подписи для оси X
        for (int n = minX; n <= maxX; n += gridStep) {
            if (n == 0) continue;
            ...
            if (x > PADDING && x < getWidth() - PADDING) {
                ...
                // Добавляем небольшие отметки на оси
                g2d.drawLine(x, centerY - 3, x, centerY + 3);
            }
        }

        // Рисуем подписи для оси Y
        for (int n = minY; n <= maxY; n += gridStep) {
            if (n == 0) continue;
        ...
        if (centerX > PADDING && centerX < getWidth() - PADDING &&
                centerY > PADDING && centerY < getHeight() - PADDING) {
    ...
    private void drawGrid(Graphics2D g2d) {
        ...
        // Вычисляем диапазон для отображения сетки
        int minX = (int) ((-centerX + PADDING) / scale);
        int maxX = (int) ((getWidth() - centerX - PADDING) / scale);
        int minY = (int) ((-getHeight() + centerY + PADDING) / scale);
        int maxY = (int) ((centerY - PADDING) / scale);
